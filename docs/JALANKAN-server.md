# Menjalankan Stechoq Pulse (v2) di server — via IP dulu, domain belakangan

Panduan menjalankan **monitoring v2** di server pakai PM2, diakses lewat **`http://<IP-server>:10102`**.
nginx + domain menyusul (lihat `../v2/README.md` bagian 5). Builder & v1 **tidak** dijalankan di sini.
...
```bash
git clone <repo-url> serversidemonitor && cd serversidemonitor
...
### 2. Daftar tempat
```bash
nano v2/locations.json       # id, name, ws (sumber WS pabrik), scene3d, layout2d
```

### 3. Akses lewat IP (sebelum ada nginx)
Buka `v2/ecosystem.config.js`, pastikan baris `V2_HOST` **dikomentari** (lihat bagian ⚡ di atas).
Default repo sudah nonaktif → dengar semua interface.

### 4. Buat akun login (WAJIB)
```bash
mkdir -p v2/logs
node v2/adduser.js admin     # ketik password (tersembunyi). Tambah akun lain = ulangi
```
...
pm2 status                   # stechoq-pulse = online
```
...
## Perintah harian
```bash
pm2 logs stechoq-pulse           # lihat log (Ctrl-C keluar)
npm run pulse:restart            # sesudah edit locations.json / taruh scene / tambah akun
npm run pulse:stop
pm2 status
```
> Sesudah **edit `ecosystem.config.js`** (mis. ubah port/host), pakai `pm2 delete stechoq-pulse && pm2 start v2/ecosystem.config.js` agar env baru terbaca — bukan `restart` biasa.
...
| Halaman langsung minta login | Normal. Belum punya akun? `node v2/adduser.js <username>`. |
...
1. Aktifkan lagi `V2_HOST: "127.0.0.1"` di `ecosystem.config.js` → `pm2 delete stechoq-pulse && pm2 start v2/ecosystem.config.js && pm2 save`.
2. Pasang `../v2/deploy/nginx-pulse.conf.example` (ganti `server_name`) → `sudo certbot --nginx -d <domain>` untuk HTTPS.
3. Detail di `../v2/README.md` bagian **5**.
